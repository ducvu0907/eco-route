...
export const readNotification = async (notifcationId: string): Promise<ApiResponse<NotificationResponse[]>> => {
  const { data } = await axiosInstance.post(`/notifications/${notifcationId}/read`);
  return data;
}
